...
public class UserInvestController extends BaseTradeController {
    ...
    @ApiParam(required = true, name = "repaymentInfoList", value = "第三方查询出借信息,获取回款记录")
    @ApiOperation(value = " 第三方查询出借信息,获取回款记录", httpMethod = "POST", notes = " 第三方查询出借信息,获取回款记录")
    public ResultApiBean repayList(@RequestBody ApiRepayListRequestBean bean) {
        logger.info("----------------------第三方查询出借信息,获取回款记录ApiRepayListRequestBean："+JSONObject.toJSON(bean));
        // 验证
