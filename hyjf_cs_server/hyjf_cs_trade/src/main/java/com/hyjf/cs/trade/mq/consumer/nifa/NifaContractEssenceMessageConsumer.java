 ...
 */
@Component
public class NifaContractEssenceMessageConsumer extends Consumer {
    ...
    public void init(DefaultMQPushConsumer defaultMQPushConsumer) throws MQClientException {
        ...
        defaultMQPushConsumer.setConsumerGroup(MQConstant.CONTRACT_ESSENCE_GROUP);
        // 订阅指定MyTopic下tags等于MyTag
        defaultMQPushConsumer.subscribe(MQConstant.CONTRACT_ESSENCE_TOPIC, "*");
        // 设置Consumer第一次启动是从队列头部开始消费还是队列尾部开始消费
        // 如果非第一次启动，那么按照上次消费的位置继续消费
        defaultMQPushConsumer.setConsumeFromWhere(ConsumeFromWhere.CONSUME_FROM_LAST_OFFSET);
        // 设置为集群消费(区别于广播消费)
        defaultMQPushConsumer.setMessageModel(MessageModel.CLUSTERING);
        defaultMQPushConsumer.registerMessageListener(new NifaContractEssenceMessageConsumer.MessageListener());
