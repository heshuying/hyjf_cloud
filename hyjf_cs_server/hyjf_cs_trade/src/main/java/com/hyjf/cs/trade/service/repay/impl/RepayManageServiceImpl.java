...
public class RepayManageServiceImpl extends BaseTradeServiceImpl implements RepayManageService {
	...
	public void checkForBankBalance(WebViewUserVO user, RepayBean repayBean) {
		...
		if (repayBean.getRepayAccountAll().compareTo(accountVO.getBankBalance()) == 0
				|| repayBean.getRepayAccountAll().compareTo(accountVO.getBankBalance()) == -1) {
			// ** 垫付机构符合还款条件，可以还款 *//*
			// 查询用户在银行的电子账户
			BigDecimal userBankBalance = getBankBalancePay(user.getUserId(), user.getBankAccount());
			if (repayBean.getRepayAccountAll().compareTo(userBankBalance) == 0
					|| repayBean.getRepayAccountAll().compareTo(userBankBalance) == -1) {
				// ** 用户符合还款条件，可以还款 *//*
				;
			} else {
				// 银行账户余额不足
	 ...
	 */
	@Override
	public File createAgreementPDFFileRepay(HttpServletRequest request, HttpServletResponse response, String borrowNid,
			String nid, String flag, Integer userId) {
		...
		if (tenderAgreementsNid != null && tenderAgreementsNid.size() > 0) {
			...
			if (borrowCustomizeVO != null) {
				// 放款记录创建时间（放款时间）
				addTime = (borrowCustomizeVO.getCreateTime() == null ? 0
						: GetDate.getTime10(borrowCustomizeVO.getCreateTime()));
	 ...
	 */
	@Override
	public Integer insertRepayOrgFreezeLog(Integer userId, String orderId, String account, String borrowNid,
			RepayBean repay, String userName, boolean isAllRepay) {
		BorrowAndInfoVO borrow = amTradeClient.getBorrowByNid(borrowNid);
		...
		String periodTotal = CustomConstants.BORROW_STYLE_ENDDAY.equals(borrowStyle)
				|| CustomConstants.BORROW_STYLE_END.equals(borrowStyle) ? "1" : repay.getBorrowPeriod();
		int remainRepayPeriod = CustomConstants.BORROW_STYLE_ENDDAY.equals(borrowStyle)
				|| CustomConstants.BORROW_STYLE_END.equals(borrowStyle) ? 1 : repay.getRepayPeriod();
	 ...
	 */
	@Override
	public WebResult getBalanceFreeze(WebViewUserVO userVO, String borrowNid, RepayBean repayBean, String orderId,
			String account, WebResult webResult, boolean isAllRepay) {
		...
		if (StringUtils.isBlank(respCode) || !BankCallConstant.RESPCODE_SUCCESS.equals(respCode)) {
			...
			logger.error("调用还款申请冻结资金接口失败:" + callBackBean == null ? ""
					: callBackBean.getRetMsg() + "订单号:" + callBackBean == null ? "" : callBackBean.getOrderId());
