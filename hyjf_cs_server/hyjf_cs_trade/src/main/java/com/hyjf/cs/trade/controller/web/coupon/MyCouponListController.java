...
//        }
    ...
    public void download(@PathVariable  Integer userId, HttpServletRequest request, HttpServletResponse response){
        ...
        try {
            QRCodeUtil.encode(systemConfig.getWechatInviteUrl() + "?refferUserId=" + userId,String.valueOf(userId),systemConfig.getFilePhysicalPath() + systemConfig.getFileUploadRealPath(), false);
        } catch (Exception e) {
            e.printStackTrace();
        }
