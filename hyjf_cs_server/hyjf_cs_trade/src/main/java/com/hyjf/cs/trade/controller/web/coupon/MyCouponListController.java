...
//        }
    ...
    public void download(@PathVariable  Integer userId, HttpServletRequest request, HttpServletResponse response){
        ...
        try {
            QRCodeUtil.encode(systemConfig.frontHost + "/landingPage/init.do?refferUserId=" + userId,String.valueOf(userId),systemConfig.getFilePhysicalPath() + systemConfig.getFileUploadRealPath(), false);
        } catch (Exception e) {
            e.printStackTrace();
        }
