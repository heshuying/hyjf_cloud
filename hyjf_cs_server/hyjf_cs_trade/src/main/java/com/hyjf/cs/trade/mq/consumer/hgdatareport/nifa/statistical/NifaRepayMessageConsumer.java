...
import com.hyjf.cs.trade.service.consumer.hgdatareport.nifa.NifaTwoRepayInfoMessageService;
...
public class NifaRepayMessageConsumer implements RocketMQListener<MessageExt>, RocketMQPushConsumerLifecycleListener {
    ...
    @Autowired
    NifaTwoRepayInfoMessageService nifaTwoRepayInfoMessageService;
    ...
    public void onMessage(MessageExt message) {
        ...
        try {
            ...
            // --> 拉数据
            // 获取标的的借款详情
            BorrowAndInfoVO borrow = this.nifaTwoRepayInfoMessageService.getBorrowByNid(borrowNid);
            if (null == borrow) {
                throw new Exception(logHeader + "未获取到相关项目信息！！");
            }
            // 获取标的还款表数据
            BorrowRepayVO borrowRepay = this.nifaTwoRepayInfoMessageService.selectBorrowRepay(borrowNid);
            if (null == borrowRepay) {
                throw new Exception(logHeader + "未获取到相关项目还款信息！！");
            }
            // 获取标的放款表数据
            List<BorrowRecoverVO> borrowRecoverList = this.nifaTwoRepayInfoMessageService.selectBorrowRecoverListByBorrowNid(borrowNid);
            ...
            String bank = "";
            BankCardVO bankCard = this.nifaTwoRepayInfoMessageService.selectBankCardByUserId(borrow.getUserId());
            ...
            if ("1".equals(borrow.getCompanyOrPersonal())) {
                // 获取借款公司信息
                BorrowUserVO borrowUsers = this.nifaTwoRepayInfoMessageService.selectBorrowUsersByBorrowNid(borrowNid);
                if (null == borrowUsers) {
                ...
                // 获取CA认证信息
                CertificateAuthorityVO certificateAuthority = this.nifaTwoRepayInfoMessageService.selectCAInfoByUsername(borrowUsers.getUsername(), cardNo, 1);
                ...
                // 获取借款人信息
                BorrowManinfoVO borrowManinfo = this.nifaTwoRepayInfoMessageService.selectBorrowMainfo(borrowNid);
                ...
                // 获取借款人CA认证信息
                CertificateAuthorityVO certificateAuthority = this.nifaTwoRepayInfoMessageService.selectCAInfoByUsername(borrowManinfo.getName(), borrowManinfo.getCardNo(), 0);
                if (null != certificateAuthority && StringUtils.isNotBlank(certificateAuthority.getCustomerId())) {
                    borrower = certificateAuthority.getCustomerId();
                } else {
                    borrower = borrow.getId().toString();
            ...
            result = this.nifaTwoRepayInfoMessageService.selectDualNifaRepayInfo(historyData, repayPeriod, borrow, borrowRepay, borrowRecoverList, recoverFee, lateCounts, nifaRepayInfoEntity);
            if (result) {
                // 保存借款详情
