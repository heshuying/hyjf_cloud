...
/**
 * @Description 合规数据上报 CERT 产品信息信息上报
 * @Author nxl
 * @Date 2018/11/28 10:57
 */
@Service
@RocketMQMessageListener(topic = MQConstant.HYJF_TOPIC, selectorExpression = MQConstant.CERT_CERTLENDPRODUCT_TAG, consumerGroup = MQConstant.CERT_CERTLENDPRODUCT_GROUP)
public class CertLendProductMessageConsumer implements RocketMQListener<MessageExt>, RocketMQPushConsumerLifecycleListener {
    ...
    public void onMessage(MessageExt msg) {
        ...
        String creditTenderNid = jsonObject.getString("planNid");
        ...
        try {
            ...
            // --> 调用service组装数据
            JSONArray listRepay = certLendProductService.getPlanProdouct(creditTenderNid);
            logger.info("数据：" + listRepay.toString());
            if (null == listRepay || listRepay.size() <= 0) {
                logger.error(logHeader + "组装参数为空！！！参数为：" + creditTenderNid);
                return;
            }
            // 上送数据
            CertReportEntityVO entity = new CertReportEntityVO(thisMessName, CertCallConstant.CERT_INF_TYPE_LENDPRODUCT, creditTenderNid, listRepay);
            try {
                // 掉单用
