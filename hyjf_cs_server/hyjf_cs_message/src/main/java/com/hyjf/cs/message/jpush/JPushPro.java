...
public class JPushPro {
	protected static final Logger logger = LoggerFactory.getLogger(JPushPro.class);
	...
	public static JPushClient getClientInstance() {
		if (jpushClient == null) {
			...
			clientConfig.setApnsProduction(!PropertiesConfig.hyjfEnvProperties.isTest()); // ios
																							// 开发环境
			jpushClient = new JPushClient(PropertiesConfig.jPushProperties.getProMasterSecret(),
					PropertiesConfig.jPushProperties.getProAppKey(), null, clientConfig);
		}
		return jpushClient;
	}
