 ...
 */
package com.hyjf.cs.user.controller.web.auth.paymentauth;
...
@CrossOrigin(origins = "*")
@RestController
@RequestMapping("/hyjf-web/user/auth/paymentauthpageplus")
public class PaymentAuthPagePlusController extends BaseUserController {
    ...
    public  WebResult<Object> page(@RequestHeader(value = "wjtClient",required = false) String wjtClient,@RequestHeader(value = "userId") Integer userId, HttpServletRequest request) {
        ...
        // 拼装参数 调用江西银行
        // 同步地址  是否跳转到前端页面
        String host = BankCommonUtil.getFrontHost(systemConfig,CustomConstants.CLIENT_PC);
        ...
        // 同步地址  是否跳转到前端页面
        String retUrl = host + errorPath +"?logOrdId="+orderId+"&authType="+AuthBean.AUTH_TYPE_PAYMENT_AUTH;
        String successUrl = host + successPath +"?logOrdId="+orderId+"&authType="+AuthBean.AUTH_TYPE_PAYMENT_AUTH;
    ...
    private void checkUserMessage(UserVO user) {
        ...
        // 判断是否授权过
        CheckUtil.check(!authService.checkIsAuth(user.getUserId(), AuthBean.AUTH_TYPE_PAYMENT_AUTH),MsgEnum.ERR_AUTHORIZE_REPEAT);
    }
