 ...
 */
package com.hyjf.cs.user.controller.wechat.auth.repayauth;
 ...
 * @Date
 */
@Api(tags = {"web端-还款授权（新）"})
@CrossOrigin(origins = "*")
@RestController
@RequestMapping("/hyjf-wechat/bank/user/auth/repayauthpageplus")
public class WeChatRepayAuthPagePlusController extends BaseUserController {

    private static final Logger logger = LoggerFactory.getLogger(WeChatRepayAuthPagePlusController.class);
    ...
    public  WebResult<Object> page(@RequestHeader(value = "userId") Integer userId, HttpServletRequest request) {
        ...
        // 同步地址  是否跳转到前端页面
        String retUrl = super.getFrontHost(systemConfig,CustomConstants.CLIENT_WECHAT) + errorPath +"?logOrdId="+orderId+"&authType="+AuthBean.AUTH_TYPE_REPAY_AUTH;
        String successUrl = super.getFrontHost(systemConfig,CustomConstants.CLIENT_WECHAT) + successPath+"?logOrdId="+orderId+"&authType="+AuthBean.AUTH_TYPE_REPAY_AUTH;
        String bgRetUrl = "http://CS-USER/hyjf-wechat/bank/user/auth/repayauthpageplus/repayauthBgreturn" ;
     ...
     * @return
     */
    @ApiOperation(value = "用户还款授权异步回调", notes = "用户还款授权异步回调")
    @PostMapping(value = "/repayauthBgreturn")
    public String repayauthBgreturn(@RequestBody BankCallBean bean) {
        BankCallResult result = new BankCallResult();
        logger.info("[用户还款授权回调开始]");
        bean.convert();
