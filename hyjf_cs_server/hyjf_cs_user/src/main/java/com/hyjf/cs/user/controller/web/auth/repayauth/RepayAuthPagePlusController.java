 ...
 */
package com.hyjf.cs.user.controller.web.auth.repayauth;
...
@CrossOrigin(origins = "*")
@RestController
@RequestMapping("/hyjf-web/user/auth/repayauthpageplus")
public class RepayAuthPagePlusController extends BaseUserController {

    private static final Logger logger = LoggerFactory.getLogger(RepayAuthPagePlusController.class);
    ...
    private void checkUserMessage(UserVO user) {
        ...
        // 判断是否授权过
        CheckUtil.check(!authService.checkIsAuth(user.getUserId(), AuthBean.AUTH_TYPE_REPAY_AUTH),MsgEnum.ERR_AUTHORIZE_REPEAT);
    }
