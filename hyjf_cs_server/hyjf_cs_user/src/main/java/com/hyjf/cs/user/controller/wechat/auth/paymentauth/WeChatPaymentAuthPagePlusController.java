...
public class WeChatPaymentAuthPagePlusController extends BaseUserController {
    ...
    public  WebResult<Object> page(@RequestHeader(value = "userId") Integer userId, HttpServletRequest request) {
        ...
        // 同步地址  是否跳转到前端页面
        String retUrl = super.getFrontHost(systemConfig,CustomConstants.CLIENT_WECHAT) + errorPath +"?logOrdId="+orderId+"&authType="+AuthBean.AUTH_TYPE_PAYMENT_AUTH;
        String successUrl = super.getFrontHost(systemConfig,CustomConstants.CLIENT_WECHAT) + successPath+"?logOrdId="+orderId+"&authType="+AuthBean.AUTH_TYPE_PAYMENT_AUTH;
        String bgRetUrl = "http://CS-USER/hyjf-wechat/bank/user/auth/paymentauthpageplus/paymentauthBgreturn" ;
