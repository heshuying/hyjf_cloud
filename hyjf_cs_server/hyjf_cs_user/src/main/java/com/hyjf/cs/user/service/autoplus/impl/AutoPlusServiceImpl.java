...
public class AutoPlusServiceImpl extends BaseUserServiceImpl implements AutoPlusService {
    ...
    public BankCallBean appGetCommonBankCallBean(UserVO users, int type, String lastSrvAuthCode, String smsCode, String sign, String token) {
        ...
        // 同步调用路径
        String retUrl = systemConfig.getAppHost() + "/user/setting/authorization/result/failed?status=99&statusDesc=授权失败&logOrdId="+bean.getLogOrderId();
        String success = systemConfig.getAppHost() + "/user/setting/authorization/result/success?status=000&statusDesc=授权成功";
        // 异步调用路
