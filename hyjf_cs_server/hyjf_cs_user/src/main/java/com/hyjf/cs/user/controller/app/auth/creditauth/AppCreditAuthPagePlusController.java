 ...
 */
package com.hyjf.cs.user.controller.app.auth.creditauth;
...
@CrossOrigin(origins = "*")
@RestController
@RequestMapping("/hyjf-app/bank/user/auth/creditauthpageplus")
public class AppCreditAuthPagePlusController extends BaseUserController {

    private static final Logger logger = LoggerFactory.getLogger(AppCreditAuthPagePlusController.class);
    private static final String APP_CREDIT_AUTH = "/page";
    private static final String APP_CREDIT_SEACH_AUTH = "/seachFiledMess";
    private static final String APP_CREDIT_BG_AUTH = "/creditAuthBgreturn";
    private static final String APP_CREDIT_CLASS_NAME = "/hyjf-app/bank/user/auth/creditauthpageplus";
     ...
     * @return
     */
    @ApiOperation(value = "APP-自动债转授权", notes = "APP-自动债转授权")
    @GetMapping(value = APP_CREDIT_AUTH, produces = "application/json; charset=utf-8")
    @ResponseBody
    public  WebResult<Object> page(@RequestHeader(value = "userId") Integer userId, HttpServletRequest request) {
        logger.info("APP-自动债转授权开始,"+
                            "className:["+ this.getClass().getName() +"],"+
                            "methodPath:["+ APP_CREDIT_CLASS_NAME + APP_CREDIT_AUTH +"]");
        ...
        //同步地址
        String retUrl = BankCommonUtil.getFrontHost(systemConfig,platform)+errorPath+"?logOrdId="+orderId+"&authType="+AuthBean.AUTH_TYPE_AUTO_CREDIT+"&platform="+platform;;
        String successUrl = BankCommonUtil.getFrontHost(systemConfig,platform)+successPath+"?logOrdId="+orderId+"&authType="+AuthBean.AUTH_TYPE_AUTO_CREDIT+"&platform="+platform;;
        //异步地址
        ...
        try {
            ...
            logger.info("APP-自动债转授权 请求银行 或 插入授权日志表ht_hjh_user_auth_log 异常,详情如下:["+ e +"]");
            throw new CheckException(MsgEnum.STATUS_CE999999);
        }

        logger.info("APP-自动债转授权结束,"+
                            "className:["+ this.getClass().getName() +"],"+
                            "methodPath:["+ APP_CREDIT_CLASS_NAME + APP_CREDIT_AUTH +"]");
        return result;
    }
     ...
     * @return
     */
    @ApiOperation(value = "APP-自动债转授权异步回调", notes = "APP-自动债转授权异步回调")
    @PostMapping(APP_CREDIT_BG_AUTH)
    @ResponseBody
    public String creditAuthBgreturn(@RequestBody BankCallBean bean) {
        logger.info("APP-自动债转授权[异步回调]开始,"+
                            "className:["+ this.getClass().getName() +"],"+
                            "methodPath:["+ APP_CREDIT_CLASS_NAME + APP_CREDIT_BG_AUTH +"]");
        ...
        if(authService.checkDefaultConfig(bean, AuthBean.AUTH_TYPE_AUTO_CREDIT)){
            ...
            logger.info("APP-自动债转授权[异步回调]结束, 用户 授权期限过短或额度过低,请重新授权");
        ...
        if (user != null &&
            bean != null &&
            (BankCallConstant.RESPCODE_SUCCESS.equals(bean.get(BankCallConstant.PARAM_RETCODE)) && "1".equals(bean.getAutoCredit()))
           ) {
            try {
        ...
        logger.info("APP-自动债转授权[异步回调]结束,"+
                            "className:["+ this.getClass().getName() +"],"+
                            "methodPath:["+ APP_CREDIT_CLASS_NAME + APP_CREDIT_BG_AUTH +"]");
        result.setStatus(true);
        result.setMessage("自动债转授权成功");
