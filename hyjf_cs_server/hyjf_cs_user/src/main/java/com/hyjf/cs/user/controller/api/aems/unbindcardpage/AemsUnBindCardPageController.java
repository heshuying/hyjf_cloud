package com.hyjf.cs.user.controller.api.aems.unbindcardpage;
 ...
 * @return
 **/
@Api(value = "api端-AEMS解绑银行卡(页面调用)", tags = "api端-AEMS解绑银行卡(页面调用)")
@RestController
@RequestMapping("/hyjf-api/aems/unbindCardPage")
public class AemsUnBindCardPageController extends BaseUserController {
    @Autowired
    AemsUnBindCardService unBindCardService;
    @Autowired
    ...
    public ModelAndView userBindCardPlus(@RequestBody AemsUnbindCardPageRequestBean unbindCardPageRequestBean, HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            //验签
            if(!SignUtil.AEMSVerifyRequestSign(unbindCardPageRequestBean, "/aems/unbindCardPage/deleteCardPage")){
                logger.info("-------------------验签失败！--------------------");
            ...
            // 拼装参数 调用江西银行
            // 同步调用路径
            String retUrl = systemConfig.getServerHost()+ "/hyjf-api/aems/unbindCardPage/return"+"?acqRes="
                    + unbindCardPageRequestBean.getAcqRes() + StringPool.AMPERSAND + "callback="
                    + unbindCardPageRequestBean.getRetUrl().replace("#", "*-*-*");

            // 异步调用路
            // 异步调用路
            String bgRetUrl = "http://CS-USER/hyjf-api/aems/unbindCardPage";
