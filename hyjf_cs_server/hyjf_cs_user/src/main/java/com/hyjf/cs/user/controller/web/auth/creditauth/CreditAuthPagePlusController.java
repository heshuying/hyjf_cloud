 ...
 */
package com.hyjf.cs.user.controller.web.auth.creditauth;
...
@CrossOrigin(origins = "*")
@RestController
@RequestMapping("/hyjf-web/user/auth/creditauthpageplus")
public class CreditAuthPagePlusController extends BaseUserController {

    private static final Logger logger = LoggerFactory.getLogger(CreditAuthPagePlusController.class);
    ...
    public  WebResult<Object> page(@RequestHeader(value = "userId") Integer userId, HttpServletRequest request) {
        ...
        // 同步地址  是否跳转到前端页面
        String retUrl = super.getFrontHost(systemConfig,CustomConstants.CLIENT_PC) + errorPath +"?logOrdId="+orderId+"&authType="+AuthBean.AUTH_TYPE_AUTO_CREDIT;
        String successUrl = super.getFrontHost(systemConfig,CustomConstants.CLIENT_PC) + successPath+"?logOrdId="+orderId+"&authType="+AuthBean.AUTH_TYPE_AUTO_CREDIT;
