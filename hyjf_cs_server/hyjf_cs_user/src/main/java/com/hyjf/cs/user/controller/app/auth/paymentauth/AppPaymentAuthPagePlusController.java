...
public class AppPaymentAuthPagePlusController extends BaseUserController {
    ...
    private static final String APP_PAYMENT_AUTH = "/page";
    private static final String APP_PAYMENT_SEACH_AUTH = "/seachFiledMess";
    private static final String APP_PAYMENT_BG_AUTH = "/creditAuthBgreturn";
    private static final String APP_PAYMENT_CLASS_NAME = "/hyjf-app/bank/user/auth/paymentauthpageplus";
     ...
     * @return
     */
    @ApiOperation(value = "APP-缴费授权", notes = "APP-缴费授权")
    @GetMapping(value = APP_PAYMENT_AUTH, produces = "application/json; charset=utf-8")
    @ResponseBody
    public  WebResult<Object> page(@RequestHeader(value = "userId") Integer userId, HttpServletRequest request) {
        logger.info("APP-缴费授权开始,"+
                            "className:["+ this.getClass().getName()+"],"+
                            "methodPath:["+ APP_PAYMENT_CLASS_NAME + APP_PAYMENT_AUTH +"]");
     ...
     * @return
     */
    @ApiOperation(value = "APP-缴费授权异步回调", notes = "APP-缴费授权异步回调")
    @PostMapping(APP_PAYMENT_BG_AUTH)
    @ResponseBody
    public String paymentAuthBgreturn(@RequestBody BankCallBean bean) {
        logger.info("APP-缴费授权[异步回调]开始,"+
                            "className:["+ this.getClass().getName() +"],"+
                            "methodPath:["+ APP_PAYMENT_CLASS_NAME + APP_PAYMENT_BG_AUTH +"]");
        ...
        if(authService.checkDefaultConfig(bean, AuthBean.AUTH_TYPE_PAYMENT_AUTH)){
            ...
            logger.info("APP-缴费授权[异步回调]结束, 用户 授权期限过短或额度过低,请重新授权");
        ...
        if (user != null &&
            bean != null &&
            (BankCallConstant.RESPCODE_SUCCESS.equals(bean.get(BankCallConstant.PARAM_RETCODE)) && "1".equals(bean.getPaymentAuth()))
            ) {
            try {
                ...
                logger.info("APP-缴费授权[异步回调], 更新签约状态和日志表异常");
    ...
    private void checkUserMessage(UserVO user) {
        ...
        // 判断是否授权过
        CheckUtil.check(!authService.checkIsAuth(user.getUserId(), AuthBean.AUTH_TYPE_PAYMENT_AUTH),MsgEnum.ERR_AUTH_REPEAT);
    }
