 ...
 */
package com.hyjf.cs.user.service.duiba.impl;
...
import com.hyjf.cs.user.service.duiba.DuibaOrderListService;
...
/**
 * @author wenxin
 * @version DuibaOrderListServiceImpl, v0.1 2019/6/10 19:11
 */
@Service
public class DuibaOrderListServiceImpl extends BaseUserServiceImpl implements DuibaOrderListService {
    ...
    public VirtualResult selectCouponUserById(String orderNum) {
        ...
        // 根据兑吧订单号查询订单信息
        DuibaOrderVO duibaOrderVO = this.selectOrderByOrderId(orderNum);
        ...
        if (duibaOrderVO != null && duibaOrderVO.getUserId() != null) {
            ...
            // 根据用户id获取用户详情信息
            UserInfoVO userInfo = this.getUserInfo(duibaOrderVO.getUserId());
            // 根据用户id获取用户信息
            UserVO user = this.getUsersById(duibaOrderVO.getUserId());
            // 根据用户id获取注册时渠道名
            String channelName = this.selectChannelNameByUserId(duibaOrderVO.getUserId());
            // 根据优惠券编码查询优惠券
            CouponConfigVO configVO = this.getCouponConfig(duibaOrderVO.getCommodityCode());
            ...
            int remain = this.checkCouponSendExcess(duibaOrderVO.getCommodityCode());
            ...
            if (countType) {
                try {
                    ...
                    // 插入优惠卷信息
                    CouponUserVO couponUser = this.insertByDuibaOrder(couponUserRequest);
                    if (couponUser.getId() != null) {
                        ...
                        // 更新订单表信息插入优惠卷用户表主键id
                        this.updateOneOrderByPrimaryKey(duibaOrderVO1);
                    } else {
                        errorFlag = true;
            ...
            if (errorFlag) {
                ...
                this.updateOneOrderByPrimaryKey(duibaOrderVOError);
                // 回滚用户积分
                // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                // 根据订单号查询积分明细表
    ...
    public String activation(String orderNum, String errorMessage) {
        ...
        try {
            ...
            // 根据兑吧订单号查询订单信息
            DuibaOrderVO duibaOrderVOStr = this.selectOrderByOrderId(orderNum);
            if (duibaOrderVOStr != null) {
                ...
                if (duibaPointsVO != null) {
                    // 查询用户最新积分
                    UserVO user = this.getUsersById(duibaPointsVO.getUserId());
                    Integer points = user.getPointsCurrent() + duibaPointsVO.getPoints();
                    if (user != null) {
                        // 更新积分
                        UserVO userUp = new UserVO();
                        userUp.setUserId(user.getUserId());
                        userUp.setPointsCurrent(points);
                        int userUpCont = this.updateUserById(userUp);
                        if (userUpCont == 0) {
                ...
                if (productType.equals(duibaOrderVO.getProductType())) {
                    ...
                    // 设置优惠卷用户表无效
                    int couponUpCount = this.deleteCouponUserById(couponUserBeanRequest);
                    if (couponUpCount == 0) {
                ...
                // 根据订单信息更新订单状态
                res = this.updateOneOrderByPrimaryKey(duibaOrderVO);
                if (res == 0) {
    ...
    public String success(String orderNum) {
        ...
        if (duibaOrderVOStr != null) {
            ...
            int orderFlag = this.updateOneOrderByPrimaryKey(duibaOrderVO);
