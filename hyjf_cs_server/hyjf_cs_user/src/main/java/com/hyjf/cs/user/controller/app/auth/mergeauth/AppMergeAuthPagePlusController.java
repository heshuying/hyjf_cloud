...
public class AppMergeAuthPagePlusController extends BaseUserController {
    ...
    public  WebResult<Object> page(@RequestHeader(value = "userId", required = false) Integer userId, HttpServletRequest request) {
        ...
        //同步地址
        String retUrl = BankCommonUtil.getFrontHost(systemConfig,platform)+errorPath+"?logOrdId="+orderId+"&authType="+AuthBean.AUTH_TYPE_MERGE_AUTH+"&platform="+platform;;
        String successUrl = BankCommonUtil.getFrontHost(systemConfig,platform)+successPath+"?logOrdId="+orderId+"&authType="+AuthBean.AUTH_TYPE_MERGE_AUTH+"&platform="+platform;;
        //异步地址
