...
//								}
			...
			if (isRepayOrgFlag == 1 && isApicronRepayOrgFlag == 1) {
				// 调用相应的担保机构还款接口
				logger.info("【直投还款】借款编号：{}，批次代偿接口请求开始！", borrowNid);
				if (isMonth) {
				...
				// 调用相应的非担保机构还款接口
				logger.info("【直投还款】借款编号：{}，批次还款接口请求开始！", borrowNid);
				if (isMonth) {
	...
	private boolean requestDebtEnd(Integer userId, JSONObject repayDetail, String orgOrderId, BorrowInfo borrowInfo) {
		...
		logger.info("【直投还款】标的：{}，结束债权。借款人：{}-{}，出借人：{}-{}，授权码：{}，原始订单号：{}。",
	...
	public int updateBorrowStatus(BorrowApicron apicron, Borrow borrow, BorrowInfo borrowInfo) throws Exception {
		...
		if (isMonth) {
			...
			logger.info("【直投还款】借款编号：{}，失败笔数为：{}", borrowNid, failCount);
			if (failCount == 0) {
				repayType = TYPE_WAIT_YES;
				repayStatus = 1;
				repayYesTime = nowTime;
...
//				CommonSoaUtils.couponRepay(borrowNid, periodNow);
        ...
        try {
			...
			logger.error("【直投还款/借款人】发送运营数据更新MQ失败！还款标的：{}", borrowNid);
        }
		logger.info("【直投还款/借款人】更新借款人的还款数据结束。借款编号：{}，还款期数：{}", apicron.getBorrowNid(), periodNow);
	...
	public boolean updateTenderRepay(BorrowApicron apicron, Borrow borrow, BorrowInfo borrowInfo, BorrowRecover borrowRecover, JSONObject repayDetail,boolean creditEndAllFlag) throws Exception {
		...
		logger.info("【直投还款/出借人】开始更新出借人相关的还款数据。借款编号：{}", apicron.getBorrowNid());
		/** 还款信息 */
		// 当前时间
		int nowTime = GetDate.getNowTime10();
		// 借款编号
		...
		logger.info("【直投还款/出借人】更新出借人的还款数据结束。借款编号：{}，还款订单号：{}", apicron.getBorrowNid(), repayOrderId);
		return true;
	}
