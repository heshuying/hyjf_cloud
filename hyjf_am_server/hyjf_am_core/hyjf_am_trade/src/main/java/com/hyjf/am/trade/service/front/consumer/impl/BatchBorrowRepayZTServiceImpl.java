...
//								}
            ...
            if (isMonth) {
                ...
                if (isRepayOrgFlag == 1 && isApicronRepayOrgFlag == 1) {
                    ...
                    // 调用相应的非担保机构还款接口
                    resultMap = this.requestRepay(borrow, borrowInfo, repayAccountId, apicron, null, recoverPlanList, creditRepayList);
                }
			} else {
                // 如果是垫付机构还款
                if (isRepayOrgFlag == 1 && isApicronRepayOrgFlag == 1) {
                    ...
                    // 调用相应的非担保机构还款接口
                    resultMap = this.requestRepay(borrow, borrowInfo, repayAccountId, apicron, recoverList, null, creditRepayList);
                }
			}
			...
			if (borrowRecoverList != null && borrowRecoverList.size() > 0) {
				...
				for (int i = 0; i < borrowRecoverList.size(); i++) {
					...
					if (creditAmount.compareTo(BigDecimal.ZERO) > 0) {
						...
						if (creditRepayList != null && creditRepayList.size() > 0) {
							...
							if (creditRepayAllFlag) {
								...
								if (borrowRecover.getRecoverCapital().compareTo(borrowRecover.getCreditAmount()) > 0) {
									...
									if (txState.equals(BankCallConstant.BATCH_TXSTATE_TYPE_SUCCESS)) {
										try {
											boolean tenderRepayFlag = ((BatchBorrowRepayZTService)AopContext.currentProxy()).updateTenderRepay(apicron, borrow, borrowInfo, borrowRecover, repayDetail, creditEndAllFlag);
						...
						if (txState.equals(BankCallConstant.BATCH_TXSTATE_TYPE_SUCCESS)) {
							try {
								boolean tenderRepayFlag = ((BatchBorrowRepayZTService)AopContext.currentProxy()).updateTenderRepay(apicron, borrow, borrowInfo, borrowRecover, repayDetail, true);
								// 结束债转已在updateTenderRepay中处理 update by wgx in 2019/01/10
							} catch (Exception e) {
								logger.error("还款中发生系统", e);
								continue;
		...
		try {
			if (!isMonth || (isMonth && periodNext == 0)) {
				...
				if (debtOverFlag) {
					...
					if (!debtStatusFlag) {
						logger.error("【直投还款】更新相应的债转为债权结束失败!" + "[承接用户：" + assignUserId + "]" + "[承接订单号：" + assignOrderId + "]" + "[还款期数：" + periodNow + "]");
						return false;
					}
				} else {
					logger.error("【直投还款】结束债权失败!" + "[承接用户：" + assignUserId + "]" + "[承接订单号：" + assignOrderId + "]" + "[还款期数：" + periodNow + "]");
					return false;
				}
			}
		} catch (Exception e) {
...
//							}
							...
							logger.info(productId+" 直投还款结束债权  借款人: "+borrowUserId+"-"+forAccountId+" 出借人: "+userId+"-"+accountId+" 授权码: "+authCode+" 原始订单号: "+orgOrderId);
...
//		accountList.setInterest(BigDecimal.ZERO); // 利息
		...
		if (!borrowRecoverFlag) {
			...
			logger.info("---------------------------------cwyang 标的号:" + borrowNid + ",还款订单号:" + borrowRecover.getRepayOrdid() + ",出借数据变更成功!");
		}
		if (borrowRecover.getCreditAmount().compareTo(BigDecimal.ZERO) > 0) {
			// 查询相应的债权转让
...
//				rabbitTemplate.convertAndSend(RabbitMQConstants.EXCHANGES_NAME, RabbitMQConstants.ROUTINGKEY_WRB_CALLBACK_NOTIFY, JSONObject.toJSONString(params));
		...
		try{
			...
			logger.info("结束债权异常!" + "[出借订单号：" + tenderOrderId + "]" + "[还款期数：" + periodNow + "]");
			return false;
		}
		return true;
	}
...
//				borrowRepayPlan.setRepayStep(4);
     ...
     * @date 2018/10/18
     */
    private JSONObject getSubJson(JSONObject subJson, String accountId, String borrowNid, String recoverRepayOrderId, BigDecimal txAmount, BigDecimal intAmount, BigDecimal serviceFee, String authCode, Account bankOpenAccount) {
                ...
                if (Validator.isNotNull(repayResult)) {
                    ...
                    if (BankCallConstant.RECEIVED_SUCCESS.equals(received)) {
                        try {
                            ...
                            logger.info("【直投批次代偿】-----------------------------------[用户ID：" + userId + "]," + "[借款编号：" + borrowNid + "],还款请求成功后,变更任务状态异常!");
                        }
                        map.put("result", repayResult);
                        map.put("delFlag", true);
                        return map;
