...
public class RepayManageServiceImpl extends BaseServiceImpl implements RepayManageService {
     ...
     */
    @Override
    public int countUserRepayedListTotal(RepayListRequest projectBean){
        Map<String, Object> params = new HashMap<String, Object>();
        String userId = StringUtils.isNotEmpty(projectBean.getUserId()) ? projectBean.getUserId() : null;
        String roleId = StringUtils.isNotEmpty(projectBean.getRoleId()) ? projectBean.getRoleId() : null;
        String status = StringUtils.isNotEmpty(projectBean.getStatus()) ? projectBean.getStatus() : null;

        String startDate = StringUtils.isNotEmpty(projectBean.getStartDate()) ? projectBean.getStartDate() : null;
        String endDate = StringUtils.isNotEmpty(projectBean.getEndDate()) ? projectBean.getEndDate() : null;

        String borrowNid = StringUtils.isNotEmpty(projectBean.getBorrowNid()) ? projectBean.getBorrowNid() : null;
        String repayStatus = StringUtils.isNotEmpty(projectBean.getRepayStatus()) ? projectBean.getRepayStatus() : null;
        params.put("userId", userId);
        ...
        if (roleId != null && "3".equals(roleId)) {
            // 垫付机构
            total = repayManageCustomizeMapper.countOrgRepayProjectRecordTotal(params);
        } else {
            // 借款人
            total = repayManageCustomizeMapper.countUserRepayProjectRecordTotal(params);
        }
        return total;
    }
     ...
     */
    @Override
    public List<RepayListCustomizeVO> searchUserRepayList(RepayListRequest projectBean, int limitStart, int limitEnd){
        Map<String, Object> params = new HashMap<String, Object>();
        String userId = StringUtils.isNotEmpty(projectBean.getUserId()) ? projectBean.getUserId() : null;
        String roleId = StringUtils.isNotEmpty(projectBean.getRoleId()) ? projectBean.getRoleId() : null;
        String status = StringUtils.isNotEmpty(projectBean.getStatus()) ? projectBean.getStatus() : null;
        ...
        String repayTimeOrder = StringUtils.isNotEmpty(projectBean.getRepayOrder()) ? projectBean.getRepayOrder() : null;
        String checkTimeOrder = StringUtils.isNotEmpty(projectBean.getCheckTimeOrder()) ? projectBean.getCheckTimeOrder() : null;

        params.put("userId", userId);
        params.put("roleId", roleId);
        params.put("status", status);
        params.put("repayStatus", projectBean.getRepayStatus());
        ...
        List<RepayListCustomizeVO> list = null;
        if (roleId != null && "3".equals(roleId)) {
            // 垫付机构 selectOrgRepayProjectList
            list = repayManageCustomizeMapper.selectOrgRepayList(params);
            if (list!=null) {
                for (int i = 0; i < list.size(); i++) {
            ...
            //selectUserRepayProjectList
            list = repayManageCustomizeMapper.selectRepayList(params);
            if (list != null && list.size() > 0) {
                for (int i = 0; i < list.size(); i++) {
