...
//		int projectType = borrow.getProjectType();// 项目类型
    ...
    public boolean updateCreditForAutoTender(String creditNid, String accedeOrderId, String planNid, BankCallBean bean,
        ...
        // 调用MQ,生成计划债权转让协议
        planCreditGenerateContractByMQ(credit.getBorrowNid(),bean.getOrderId());
        result = true;
        return result;
    }
...
//				assignCapital = HTJServiceFeeUtils.getAssignCapital(account, sellerCapitalWait, sellerInterestAdvanceWait, sellerCapital, sellerInterestAdvance);
        ...
        if (flag) {
            ...
            throw new RuntimeException("债转日志debtCrditTenderLog表保存失败，债权标号：" + creditNid + ",出借编号：" + sellOrderId);
        }
    }
...
//						}
     ...
     * @param assignNid
     */
    private void planCreditGenerateContractByMQ(String borrowNid,String assignNid) {
        try {
