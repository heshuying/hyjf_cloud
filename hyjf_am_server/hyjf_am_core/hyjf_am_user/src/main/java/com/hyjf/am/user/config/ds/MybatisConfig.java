...
import com.hyjf.am.user.interceptor.SyncAccountInterceptor;
...
public class MybatisConfig {
	...
	public DynamicDataSource dynamicDataSource(@Qualifier("writeDataSource") DataSource writeDataSource,
        ...
        DynamicDataSourceContextHolder.dataSourceKeys.add(DbType.READ2.name());
        
        // 将 Slave 数据源的 key 放在集合中，用于轮循
        DynamicDataSourceContextHolder.slaveDataSourceKeys.add(DbType.READ1.name());
        DynamicDataSourceContextHolder.slaveDataSourceKeys.add(DbType.READ2.name());
	...
	public SqlSessionFactory sqlSessionFactory(@Qualifier("dynamicDataSource") DynamicDataSource dynamicDataSource,
											   @Value("${mybatis.mapper-locations}") String mapperLocations, @Autowired SyncRuserInterceptor syncRuserInterceptor, @Autowired SyncAccountInterceptor syncAccountInterceptor) throws Exception {
		...
		factoryBean.setPlugins(new Interceptor[] {syncRuserInterceptor,syncAccountInterceptor});
