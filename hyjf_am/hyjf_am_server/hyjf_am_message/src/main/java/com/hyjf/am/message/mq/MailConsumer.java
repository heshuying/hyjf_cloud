...
public class MailConsumer extends Consumer {
	...
	@Value("${rocketMQ.group.mailCodeGroup}")
	private String mailCodeGroup;
	@Value("${rocketMQ.topic.mailCodeTopic}")
	private String mailCodeTopic;

	@Override
	public void init(DefaultMQPushConsumer defaultMQPushConsumer) throws MQClientException {
		defaultMQPushConsumer.setInstanceName(String.valueOf(System.currentTimeMillis()));
		defaultMQPushConsumer.setConsumerGroup(mailCodeGroup);
		// 订阅指定MyTopic下tags等于MyTag
		defaultMQPushConsumer.subscribe(mailCodeTopic, "*");
		...
		public ConsumeConcurrentlyStatus consumeMessage(List<MessageExt> msgs, ConsumeConcurrentlyContext context) {
			...
			if (null != message) {
				...
				case MessageConstant.MAILSENDFORMAILINGADDRESS:// 给指定用户发送邮件
					MailHandle.sendMail(mailMessage.getToMailArray(), mailMessage.getSubject(), mailMessage.getMailKbn(),
