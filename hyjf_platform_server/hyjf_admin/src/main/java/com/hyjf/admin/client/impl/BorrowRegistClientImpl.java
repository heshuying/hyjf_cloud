...
public class BorrowRegistClientImpl implements BorrowRegistClient {
    ...
    public JSONObject updateBorrowRegist(String borrowNid){
        String url = "http://CS-TRADE/cs-trade/borrow_regist/update_borrow_regist/" + borrowNid;
        AdminResponse response = restTemplate.getForEntity(url,AdminResponse.class).getBody();
        if(response!=null){
            JSONObject jsonObject = new JSONObject();
            jsonObject.put("status",response.getRtn());
            jsonObject.put("msg",response.getMessage());
            return jsonObject;
        }
        return null;
    }
