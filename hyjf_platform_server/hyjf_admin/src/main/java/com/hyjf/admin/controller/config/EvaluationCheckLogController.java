 ...
 * @return
 **/
@Api(tags = "配置中心-风险测评开关配置操作日志")
@RestController
@RequestMapping("/hyjf-admin/evaluationchecklog")
public class EvaluationCheckLogController extends BaseController {

	@Autowired
	private EvaluationCheckLogService service;

	public static final String PERMISSIONS = "evaluationchecklog";

	@ApiOperation(value = "风险测评开关配置操作日志", notes = "风险测评开关配置操作日志")
	@PostMapping("/init")
	@AuthorityAnnotation(key = PERMISSIONS, value = ShiroConstants.PERMISSION_VIEW)
	public AdminResult<ListResult<EvaluationCheckLogConfigVO>> getRecordList(@RequestBody EvaluationCheckLogRequest requestBean) {
		EvaluationCheckLogResponse response = service.getEvaluationCheckLogList(requestBean);
		if (response == null) {
